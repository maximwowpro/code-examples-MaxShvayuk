...
{
	SquareArray *masiv = malloc(sizeof(SquareArray)*Asize);//dynamic allocation of memory
...
{// *aList - ARRAY of Squares, where we add a square 
 //  *aSquare - a figure type square
	...
	if(aList!= NULL && aSquare!=NULL)
	{
		...
		{
			Square *figure = &(aList->array[aList->current_size]);//refer to CURRENT_SIZE element ARRAY of Squares			
