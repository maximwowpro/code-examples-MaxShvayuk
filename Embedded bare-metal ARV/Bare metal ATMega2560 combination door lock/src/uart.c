...
void uart_init(void)
{
	cli(); /* the Global Interrupt Flag should be cleared (and interrupts globally disabled) when doing the USART initialization */
...
void uart_print_commands_list(void)
{
	...
	uart_print_str("open (user)                       - unlock the lock for a few time\n");
	uart_print_str("close (user)			  - lock the lock\n");
...
bool uart_check_user_pass(uint8_t num_keys, uint8_t *key_arr)
{
	...
	uart_print_str("\nEnter user's password (each number separated by spaces):\n ");
	...
	if (false == is_user_key_valid(num_keys, key_arr)) {
		uart_print_str("Invalid password, you entered: \n");
		uart_print_str(rdbuff);
		rxcflag = false;
		return false;
...
void uart_read_command(lock_st *lock)
{
	while (rxcflag) {
		if (uart_compare_with_rdbuff("watch all keys")) {
			if (false == uart_check_admin_pass(lock->key))
				continue;

			for (uint8_t i = 1; i < lock->num_keys + 1; i++) {
				uart_print_uint8_dec(i);
				uart_print_str(" key =    ");
				eeprom_read_key(i, lock->key);
				uart_print_uint8_key_hex(lock->key);
			}
		}
			...
			if (false == uart_read_pass(lock->key)) {
				uart_print_str("Invalid password format, you entered: \n");
				uart_print_str(rdbuff);
				rxcflag = false;
				continue;
			}
			eeprom_update_admin_key(lock->key);
			...
			eeprom_write_user_key(&(lock->num_keys), lock->key);
			...
			eeprom_read_key(key_to_delete, lock->key);
			...
			if (false == uart_check_user_pass(lock->num_keys, lock->key))
				continue;
			
			uart_print_str("UART: you entered valid password, unlock...\n");
			...
			uart_print_str("UART: you entered valid password, locking...\n");
			led_RGB_red (&(lock->led));
			/* move the stepper to close the door */
